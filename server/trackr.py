...
CORS(app, origins=["*api.ncf.space*", "*localhost*"])
...
def oauth_callback(provider):
    ...
    if is_ok:
        ...
        post_user(username, str(provider), email, str(provider))
        user = user(nickname)
        login_user(user)
    return flask.render_template('index.html')
...
@app.route('/api_post/<username>/<password>/<email>/',
           defaults={'provider': 'Trackr'}, methods=['POST'])
def post_user(username, password, email, provider):
    payload = {'user_id': username, 'password': sha1(password).hexdigest(),
